...
void global_map_cartesian::init_map(double d_x, double d_y, double d_z, int n_x, int n_y, int n_z, double min_z)
{
    this->map_dx = d_x;
    this->map_dy = d_y;
    this->map_dz = d_z;
...
bool global_map_cartesian::xyz2xyzIdxwithBoderCheck(Vec3 xyz_l, Vec3I &xyz_idx)
{
    ...
    double z = xyz_l(2)-this->map_min_z;
    int z_idx =  static_cast<int>(z/this->map_dz);
    cout << xyz_l.transpose() << "--->" << x_idx << "," << y_idx << "," << z_idx << endl;
    if(x>0 && y>0 && z>0)
    {
        if( x>0 && y>0 && z>0
...
void global_map_cartesian::input_pc_pose(vector<Vec3> PC_l, SE3 T_wl)
{
